...
def generate_lookahead_tables(mjd0=59560.2, mjd_max=59621.2, timestep=5., timestep_max=20.,
    ...
    for i, mjd in enumerate(mjds):
        ...
        if sm.sunAlt <= sunLimit:
            ...
            sky_brightness[u'moonangle'][i] = moonangles > 0.698 #placeholder moon angle limit, ~40 degrees
            vmjd[i] = mjd
...
if __name__ == "__main__":
    ...
    # 2-months
    mjds = np.arange(59853, 59853+366*nyears+366/6., 366/6.)
    count = 0
    for mjd1, mjd2 in zip(mjds[:-1], mjds[1:]):
        ...
        outfile="lookahead_pre_seg_"+str(count).zfill(3), outpath=None,fieldID=True)
        count += 1

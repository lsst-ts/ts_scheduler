...
class Model:
    ...
    async def validate_observing_blocks(
        ...
        for block_id in self.observing_blocks:
            target_validate = DriverTarget(
                observing_block=self.observing_blocks[block_id].model_copy(deep=True)
            )
    ...
    async def _handle_load_observations_from_db(self, database_path: str) -> None:
        ...
        self.log.info(
            "Updating telemetry (and conditions) before playing back observations."
        )
        self.models["observatory_state"].time = utils.astropy_time_from_tai_unix(
            utils.current_tai()
        ).unix
